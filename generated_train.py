...
def load_data_from_file(filename):
	...
	with open(filename) as f:
		for n, line in enumerate(f):
			data = bin(int(line.strip().split(',')[1]))[2:].zfill(128**2)
			parsed_image = [[int(p) for p in data[i*128:(i+1)*128]] for i in range(128)]
			x_data.append(parsed_image)
			y_data.append(line.strip().split(',')[0])
	return np.asarray(x_data), np.asarray(y_data)

print('Loading image data')
x_train, y_train = load_data_from_file('training_data/capture/output_train.csv')
x_test, y_test = load_data_from_file('training_data/capture/output_test.csv')
