...
def write_data(frame, letter):
    ...
    with open('training_data/capture/output.csv', 'a') as f:
        f.write(f'{alpha.index(letter.lower())},{int(data_string,2)}\n')
...
while ret:
    ...
    downscale_edges = cv2.Canny(downscale,100,120)
    ...
    if mode in ['random', '1']:
        ...
        printed = True
        if k%256 == 32:
            write_original_image(cv2, raw_image, key)
            write_data(downscale_edges, key)
            printed = False
